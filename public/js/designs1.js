$(function () {
    ...
    function cellResize(numCells, cellResize) {
        ...
        if (cellResize <= maxCellSize) {
            $("td").css({
                width: cellResize + "px",
                height: cellResize + "px",
            });
            $("tr").css({ height: cellResize + "px" });
        } else {
            $("td").css({
                width: maxCellSize + "px",
                height: maxCellSize + "px",
            });
    ...
    $("#pixel_canvas").on("mouseenter", "td", function () {
        if (down) {
            //colors only when mouse button is pressed
            let pickerColor = colorInput.val();
            if ($(this).hasClass("clear")) {
                $(this).css({ "background-color": pickerColor });
                $(this).removeClass("clear");
    ...
    $("#save").click(function () {
        ...
        html2canvas(document.getElementById("pixel_canvas"))
            .then((canvas) => {
                ...
                $(".images").prepend(
                    "<div class='image-box'><img src='" + img.src + "'/></div>"
                );
            })
            .catch(function () {
                setTimeout(empty, 5000);
    ...
    //load images
    function makeHtml(array, show) {
        //show parameter is for data-show attribute, which marks showed files (for show/hide functionality)
        let imagesHtml = "";
        for (let i = 0; i < array.length; i++) {
            imagesHtml =
                imagesHtml +
                "<div class='image-box' data-show='" +
                show +
                "'><img src='" +
                dir +
                array[i] +
    ...
    $.get("getdata.php", function (data) {
        //output of getdata.php is an array of file names as strings
        let added;
        files = JSON.parse(data);
        $.each(files, function (i, val) {
            if (i > 1) {
                //we leave first two values as they are '.' and '..', not image files
        ...
        $("#show").click(function () {
            //when cliked 'show more', show the rest;
            if (showed === false) {
